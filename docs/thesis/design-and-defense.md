...
本文按仓库实际实现组织，可作为论文正文和答辩讲稿的技术底稿。学校格式、作者信息、参考文献规范需按实际要求编排；最新结果引用[本轮验收报告](../test/acceptance-2026-09-09.md)，[前轮报告](../test/acceptance-2026-09-07.md)保留历史记录。可朗读的[约8分钟讲稿](defense-script-8min.md)与[证据索引](evidence-index.md)按当前已验证状态编写，未验证项目不能改写为已完成实验。
...
09-09已在Docker CI补跑11项数据库条件测试，全部通过；后端全量210项中208项通过，剩余OSS和授权照片模型两项跳过。前端CI完成55项单测、4项媒体条件守卫与5项UI契约测试。数据库测试过程中修正了最小测试上下文的JDBC/Flex事务装配、权限前缀查询和Nacos隔离；失败与成功作业均在报告中保留。
