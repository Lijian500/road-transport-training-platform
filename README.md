...
> 本机论文演示交付新增真实媒体验收入口、独立场景数据和补学恢复，前后端CI及11项数据库测试已通过。执行步骤见[媒体验收手册](docs/test/media-acceptance.md)，实际结果及剩余条件见[2026-09-09报告](docs/test/acceptance-2026-09-09.md)。
...
此前已补齐本企业车辆管理、学员学习档案、管理学时监管及两个工作台首页。只支持“仅学习”和“学习+考试”两种培训模式，均需课程与参训人员。最新功能、真实联调范围及未完成的云媒体验收见[本轮报告](docs/test/acceptance-2026-09-09.md)；运行方式见[部署手册](docs/deployment/local-demo.md)，论文配套材料见[设计与答辩材料](docs/thesis/design-and-defense.md)。
...
要求考试的计划必须关联已启用试卷，并冻结考试时长、及格分与试卷题目快照；无需考试的
计划不保存考试规则。计划课程同时冻结课件标题、顺序、时长和OSS对象引用，历史任务不
读取后续被修改的课程或试卷内容。
